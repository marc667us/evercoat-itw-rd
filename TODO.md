...
| 🔴 **I30** | **22 more `session.rollback()` calls sit inside IntegrityError handlers, across 9 modules.** | Each one rolls back the **TOPMOST** transaction and discards every nested one, so each is a latent transaction-destroyer the moment §12's reuse rule composes it into a larger unit of work. Two have already bitten: `open_failure` (would have discarded a test completion) and `record_driver` (would have discarded a cloned version) — both now use `begin_nested()` OUTSIDE the `try`. The remaining sites, measured 2026-08-21: `api/admin_reference_data.py` ×2, `api/admin_stage_gates.py` ×2, `domains/approvals/service.py` ×1, `domains/failures/service.py` ×2, `domains/formulations/service.py` ×3, `domains/laboratory/service.py` ×3, `domains/materials/service.py` ×5, `domains/messaging/service.py` ×1, `domains/testing/service.py` ×2. **They are also REDUNDANT even standalone** — `session_scope` already rolls the request back on any exception — so the sweep only removes risk. Do it as one reviewed slice with a shared savepoint helper, not opportunistically. |
