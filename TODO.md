...
| **I22** | **MSD has no HTTP route at all.** | `app/domains/msd/` contains only `retrieval.py`; nothing is registered in `main.py`, and there is no screen. The top-bar "MSD" button is a disabled placeholder. Slice 7 per the plan. Raised when the operator asked why MSD was not up. |
...
| # | Issue | Detail |
|---|---|---|
| **I20** | **`projects.project_members` has a `USING` clause and NO `WITH CHECK`, so PostgreSQL reuses `USING` — which is organization-only.** | `migrations/001_core_tenancy.sql:494-497`, and `001:522` grants INSERT on the whole `projects` schema to `evercoat_app`. So a caller executing SQL as the app role can insert **their own** `project_members` row for a `restricted` project and `core.is_project_member()` — which is SECURITY DEFINER and reads the table regardless — then answers TRUE. That does not just expose one project: **it defeats every project-scoped policy in the database at once**, which is a strictly larger hole than the messaging one migration 025 was written to close. 🔴 **Not reachable over HTTP** — `api/projects.py` requires both `project.assign_member` *and* `require_project_member()`, so you must already be inside the project. It is a database-layer gap, which is exactly the layer `SECURITY.md` §1 says must hold when the application layer does not. **Deliberately NOT fixed blind this session:** the obvious `WITH CHECK` has the same bootstrap problem migration 025 hit — a project's first member cannot be added by someone who must already be a member — and `projects.projects` has **no `created_by` column** to use as the escape hatch that worked for channels. It needs a real database to design against, and Docker on this host is wedged. |
